...
$(window).load(function() {
    ...
    var queryString=GetUrlValue('q');
    if (queryString !== undefined && queryString !== '') {
        console.log(GetUrlValue(queryString));
	$("#searchTextInput")[0].value=queryString;
            var input = $.trim($('#searchTextInput')[0].value);
...
// kkeller: use second if in order to hide workspace objects
...
function displayCategories() {
    ...
    for (var p in categoryInfo.structure) {
        if (categoryInfo.structure.hasOwnProperty(p)) {
            if (p === "authenticated") {
// kkeller: comment out to hide workspace results
...
// function added kkeller 09oct2013
// cribbed from http://javascriptproductivity.blogspot.com/2013/02/get-url-variables-with-javascript.html
